...
public class ParkingMeter {
	...
	// Secret Key 
	private static final  int[] secretKeys =        {1234,4321,9876,8888,9999,4545};
	
	// Secret Key Text
	private static final  String[] secretCommands = {"Transaction Log anzeigen", 
   ...
   private enum Messages  {
    	START_MESSAGE ("Start von ParkingMeter"), END_MESSAGE("Exit ParkingMeter -> Bye");
    	String msg;
	...
	public static void main (String[] args) {
		...
		System.out.println(getMessageText(Messages.START_MESSAGE));
		System.out.println("======================");
		System.out.println(formatSecretKeysInfo());
		addToTransactionLog (getMessageText(Messages.START_MESSAGE));
		addToTransactionLog (formatCoinContent(coinsParkingMeter));
		
		actionController();

		System.out.println(getMessageText(Messages.END_MESSAGE));
	}  	
	...
	private static void actionController(){
		...
		while (!exit) {
			...
			int parkingSpotNumber = TastaturIntRead("",-1);
	...
	/**
	 * Executes the [UserStory 1 & 2] "Payment of Parking". 
	 * The user is asked to enter the parking coins. The user input
	 * is verified and processed against the parkingDef table. 
	 * If the user pays more time than allowed,the rest of coins is returned
	 * to the customer
	...
	private static void insertCoinsController (int parkingSpotNumber){
		...
		if (checkAllIsNumeric (coinsString)) {
			...
			if (verifyValidCoins(insertedCoins)) {
				String receipt = parkingMeterPayment(parkingSpotNumber, insertedCoins);
				// TODO Erweiterung mit [UserStory 3] Beleg ausdrucken
				System.out.println("Beleg ausdrucken [j/n]");
				String input = TastaturRead.readString().trim().substring(0, 1).toLowerCase();
				if (input.startsWith("j") ) System.out.println(receipt);
			}
			else {
	...
	private static void insertNewCoinsValueController() {
		// Ausgabe neuer Inhalt 
		System.out.println("Neuer Münzbestand eingeben");
		for (int i=0 ; i <  coinsBoxValue.length; i++) {
			//Neuer Bestand in Münzeinheit [1] € 0.50
			System.out.println("Neuer Bestand in Münzeinheit [" + (i+1) + "] " + currencyString + " " + coinsBoxValue[i]);
			int input = TastaturIntRead("Die Zahl muss zwischen 1 und 100 sein!",-1);
			if (input >= 0 && input <= 100) coinsParkingMeter[i]=input;
	 ...
	 * @return the number 
	 */
	private static int TastaturIntRead(String errorMessage, int defaultValue){
		String input = TastaturRead.readString().trim();
		int zahl=defaultValue;
		try {
	 ...
	 * [2014-11-05 12:54] Start von ParkingMeter<br>
	 * [2014-11-05 12:54] Inhalt der Münzeinheiten TOTAL:  17.50 € </code><br>
	 */
	...
	// TODO Fehler beheben: Buchung der Parkgebühr auf die Münzeinheiten
	 static void bookParkingCoins (int[] parkCoins) {
		...
		for (int coinBox = 0 ; coinBox < coinsBoxValue.length ; coinBox++) {
			...
			coinsParkingMeter[coinBox] += parkCoins[coinBox];
		}
	}
	 ...
	 */
	static String parkingMeterPayment (int parkingSpotNumber, double insertedCoins[]) {
		...
		System.out.println (out);
		addToTransactionLog(out);
		String receipt="Beleg Parkhaus DHBW\n";
		receipt += new Date()+ "\n";
		receipt += "Bezahlte Gebühr € " + parkValue + "\n";
		receipt +=  "(inkl 19% MWSt = € " + parkValue * 0.19 + "\n";
		receipt +=  "MWSt-No 123.35.345\n";
		receipt +=  minutes + " Minuten bis \n";
		receipt +=  meterDate[parkingSpotNumber-1] + " \n";
		receipt +=  "Herzlichen Dank";
	...
	/**
	 * Returns a text string based on a text index
	 * @param textNummer the text index 
	 * @return the text String
	 */
	private static String getMessageText (Messages messages)  {
	...
	/**
	 * Computes from two dates the time difference in milliseconds. The time difference is always positive, 
