...
int main(void){
    ...
    //     ! this test is kind of critical, since it uses dangling pointers
    //       and is based on empirical observations
    //       currently works with linux distributions and OSX (XCode)
    ...
    mlSortIn(queue1,  mtNewThread(tid++, 1,  3));   count++;
    testListConistency(queue1);

    mlSortIn(queue1,  mtNewThread(tid++, 1, 70));   count++;
    testListConistency(queue1);

    mlSortIn(queue1,  mtNewThread(tid++, 1, 21));   count++;
    testListConistency(queue1);
    ...
    for (j = 0; j < count; j++) {
        mlSortIn(queue2,  mtNewThread(j+1, 1,  iar[j]));
    ...
    for (j = 0; j < 100; j++) {
        rnd1 = (int)random(); 
        rnd2 = (int)random();
        mlSortIn(queue1,  mtNewThread(tid++, rnd1,  rnd2));
    }
