...
int main(void)
{
    ...
    while (1) {
        ...
        uint8_t dataLen = read_display_buffer(readBuffer);
        if(dataLen > MIN_LENGTH_BUFFER_MESSAGE) 
        {
            if(readBuffer[0] == 0x1b && readBuffer[1] == 'A' && readBuffer[2] == 0x01)
            {
                if (readBuffer[3] == BUTTON1_DOWN) 
                {
                    CT_LED->BYTE.LED7_0 += 7;
                } else if (readBuffer[3] == BUTTON1_UP)
                {
                    CT_LED->BYTE.LED7_0 += 19;
                }
            }
        }
