 ...
 * Return zero on success; non-zero otherwise
 */
uint8_t cmd_touch_set_touch_font_color(uint8_t normal_color, uint8_t select_color);
 ...
 * specified by text[] on the button. When the button is touched, the respective code is
 * included in the receive message.
 * Return zero on success; non-zero otherwise
 */
...
/*
 * Resets all touch areas (if 0 passed) or the selected one (if not 0 passed) - removes from display if "remove" set to non-zero.
 */
