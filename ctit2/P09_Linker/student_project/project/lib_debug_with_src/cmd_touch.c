 ...
 */
uint8_t cmd_touch_set_touch_font(uint8_t font_number)
{
 ...
 * according to description in header file
 */
uint8_t cmd_touch_set_touch_font_zoom_factor(uint8_t x_zoom_factor,
 ...
 * according to description in header file
 */
uint8_t cmd_touch_set_touch_font_color(uint8_t normal_color,
                             uint8_t select_color)
{
    uint8_t cmd_buffer[10];

    // accept only values supported by display
    if (normal_color > 32 || select_color > 32) {     //|| backgroundColor == 0
        return 1;
    }
 ...
 * according to description in header file
 */
uint8_t cmd_touch_set_touch_panel_color(uint8_t normal_edge_inside,
                              ...
                              uint8_t select_fill)
{
    ...
    return lcd_io_write_cmd_to_display(cmd_buffer, 8);
}

/*
 * according to description in header file
 */
uint8_t cmd_touch_set_touch_enable(uint8_t on_off)
{
    uint8_t cmd_buffer[10];
 ...
 * according to description in header file
 */
uint8_t cmd_touch_define_touch_button(uint16_t left, uint16_t top,
                            ...
                            uint8_t text[])
{
    ...
    return lcd_io_write_cmd_to_display(cmd_buffer, i + 13);
}

uint8_t cmd_touch_reset_all_touch_buttons(uint8_t selected, uint8_t remove)
{
    uint8_t cmd_buffer[10];

    cmd_buffer[0] = 'A';
    cmd_buffer[1] = 'L';
    cmd_buffer[2] = selected; // all
    cmd_buffer[3] = remove;
