 ...
 * @author tebe
 */
public class XorCrackerBruteApp {
	...
	private static void run() throws IOException {
		try {
			...
			for(int c = 0; c <= getNumberOfCandidateKeys(); c++){
				int[] key = getCandidateKey(filename, keylength, c);
	...
	private static int[] getCandidateKey(String filename, int keylength, int mostFrequentValue) throws IOException {
		...
		for(int keyByte = 0; keyByte < keylength; keyByte++){
			...
			key[keyByte] = frequencyTable[keyByte].getMostFrequentByte() ^ mostFrequentValue;
		}
		return key;
	}
}
