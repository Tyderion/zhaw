...
import static serie8.MsgCode.getMessageText;
...
public class ParkingMeter {
	...
	private static void insertCoinsController (int parkingSpotNumber){
		...
		if (checkAllIsNumeric (coinsString)) {
			...
			if (verifyValidCoins(insertedCoins)) {
				...
				char in = TastaturRead.readChar();
				if (in == 'j') {
                    printCustomerReceipt(receipt);
                }
	...
	private static void printCustomerReceipt(String receipt) {
	    System.out.println(receipt);
	}

	// TODO [UserStory 8] Neuer M�nzbetrag eingeben
	/**
	 * 
	 */
	private static void insertNewCoinsValueController() {
		...
		System.out.println(getMessageText(MsgCode.ENTER_NEW_COINS));
		int i = 0;
		while (i < coinsParkingMeter.length) {
			System.out.println(String.format(getMessageText(MsgCode.ENTER_NEW_COINS_VALUE), coinsBoxValue[i]));
			long in = TastaturRead.readLong();
			if (in < 0){
				System.out.println(getMessageText(MsgCode.ERROR_NEGATIVE_COUNT_NOT_ALLOWED));
				continue;
			}
			coinsParkingMeter[i] = (int)in;
			i++;
		}
	}
	...
	static String parkingMeterPayment (int parkingSpotNumber, double insertedCoins[]) {
		...
		Date start = new Date();
		meterDate[parkingSpotNumber-1] = addTime(start,minutes);
		...
		double value= computeCoinsValue(parkCoins);
        receipt.append("Beleg Parkhaus DHBW\n");
        receipt.append("\n");
        receipt.append(formatDate(start));
        receipt.append("\n\n");
        receipt.append(String.format("Bezahlte Gebühr CHF %.2f\n", value));
        receipt.append(String.format("Inklusive 8 %% Mwst. = CHF %.2f\n", value * 0.08));
        receipt.append("\n");
        receipt.append("MWSt-No 123.35.345\n");
        receipt.append(String.format("%d Minuten bis\n", minutes));
        receipt.append(formatDate(meterDate[parkingSpotNumber-1]));
        receipt.append("\n\n");
        receipt.append("Herzlichen Dank\n");
