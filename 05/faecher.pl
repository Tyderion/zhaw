...
% 2) Beispiel: alle_voraussetzungen(jff, X) liefert [swe, ood, assit] (unsortiert)
voraussetzungen(X, Fächer):- voraussetzung(Fächer, X).
voraussetzungen(X, Fächer):- voraussetzung(A, X), voraussetzungen(A, Fächer).
alle_voraussetzungen(X,Resultat) :- setof(M,voraussetzungen(X, M),Resultat).

% 3) Beispiel: alle_nachfolger(tin2, X) liefert [pra, mc, cgr, eesy] (unsortiert)
nachfolger(X, Fächer):- voraussetzung(X, Fächer).
nachfolger(X, Fächer):- voraussetzung(X, Y), nachfolger(Y, Fächer).
alle_nachfolger(X, Resultate):- setof(M, nachfolger(X, M), Resultate).

% 4) Beispiel: module(6, X) liefert [dd2, eesy]
module(S, Fächer):- modul(Fächer), semester(Fächer, S).
